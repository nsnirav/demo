...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        memset(&sndrcv, 0, sizeof(struct sctp_sndrcvinfo));

        ret = sctp_recvmsg(sock, buffer, BUFFER_SIZE, NULL, 0, &sndrcv, &flags);

        printf("%d bytes received....stream %u\n",
                           ret, sndrcv.sinfo_stream);
    }
